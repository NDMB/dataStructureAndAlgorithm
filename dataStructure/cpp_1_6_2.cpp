...
struct singly_ll_node // 노드
{
    int data;
    singly_ll_node* next;
};

class singly_ll // 클래스 구현
{
    public:
    ...
    public:
        ...
        singly_ll(singly_ll& other) : head(NULL)
        {
            if (other.head)
            {
        ...
        singly_ll(const std::initializer_list<int>& ilist) : head(NULL) // 리스트를 이용하여 연결 리스트를 초기화 하는 방법
        {
